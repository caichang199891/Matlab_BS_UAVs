...
% 1) 场景初始化
scene = scene.generate_scene(cfg);
state = scene.init_entities(scene, cfg);

% 2) 信道初始化（稀疏Top-L路径）
ch = channel.init_sparse_paths(state, scene, cfg);

% 3) Agent 初始化（baseline 或 RL stub）
agent_state = agent.policy_baselines("init", [], cfg);

% 4) logger
log = sim.logger_init(cfg, snr_dB, mc_id, scene);
...
for t = 1:T
    ...
    % A) 真值推进：运动学 + 环境 + 信道演化
    % -------------------------
    [state, scene] = scene.step_entities(state, scene, cfg, dt);
    ch = channel.update_sparse_paths(ch, state, scene, cfg, dt);

    % -------------------------
    % B) 多模态输入：运动 + 语义（遮挡风险）
    % -------------------------
    motion = fusion.motion_predictor_kf(state, cfg);      % 可输出预测位置/速度及不确定度
    sem    = fusion.semantic_predictor_stub(state, scene, cfg); % 输出 p_blk_hat (每UAV)
    ...
    % -------------------------
    sinr_true = channel.compute_truth_sinr(ch, state, scene, cfg);
